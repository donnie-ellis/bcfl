...
const DraftPage: React.FC = () => {
  ...
  const { data: players } = useSWR<Player[]>(
    draftData ? `/api/db/league/${draftData.league_id}/players` : null,
    fetcher,
    ...
    <div className="flex flex-col h-screen">
      ...
      <Sheet open={state.isSheetOpen} onOpenChange={(open) => dispatch({ type: 'SET_IS_SHEET_OPEN', payload: open })}>
        ...
        <SheetTrigger asChild>
          <Button
            size="icon"
            className={`fixed right-4 bottom-[calc(1rem+env(safe-area-inset-bottom))] h-14 w-14 rounded-full shadow-lg md:hidden ${isCurrentUserPick ? 'animate-pulse ring-4 ring-success' : 'opacity-60 transition-opacity duration-300'}`}
          >
